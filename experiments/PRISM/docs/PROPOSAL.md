...
> **Version**: v3.3 (2026-06-12) — **M1b complete; gate adjudicated: ETT-only PRISM (§8 pivot clause)**. M1b ran the full amended protocol: DA/IC decision-loss oracle (killed — all 16 settings noise, switch ratio 0.78–0.95), realized-vol target CryptoVol (partial pass — models beat anchors 5–7%, H96 switch_ratio 0.47–0.48 but median_streak=1 everywhere; FS recovers 63–67% causally — real but bursty structure, fails strict condition 2), ETTh1 seed bands (accepted — oracle gap >> noise band), CryptoMISO pool expanded to 6 models (degeneracy unchanged). **Final gate**: general-benchmark leg (ETTh1) PASS; finance leg no surviving instantiation → ETT-only PRISM is the primary paper. CryptoVol results (strong FS recovery, bursty regime structure) retained as appendix. v3.2 (2026-06-12) recorded M1a results and amended gate protocol. v3.1 (2026-06-11) incorporated 2026 heterogeneous-MoE wave. v3.0 was the English rewrite. This document is the **single source of truth** for all subsequent coding and experiments.
>
