...
> **Version**: v3.7 (2026-06-14) — **M3 complete; dynamic β + drift loop passed narrowly (`m3-dynamic-beta-drift-loop`)**. The pivot system improved stress-weighted loss on ETTh1/ETTm2/Weather and β was nontrivial on all three, but the tuned drift gain was 0.0 throughout. Interpretation: dynamic β is useful as a descriptor-derived weighting signal; the current drift detector does not yet add share-rate adaptation value. v3.6 (2026-06-14): **M2 complete; router viability gate failed on ETTm2 (`m2-router-viability`)**. The minimum viable PRISM router beat descriptor ridge on ETTh1/ETTm2/Weather and beat Fixed-Share on ETTh1/Weather, but failed the preregistered kill criterion on ETTm2 (PRISM 0.073668 vs Fixed-Share 0.060931). The route pivots: frozen heterogeneous experts + Fixed-Share become the robust causal tracker; PRISM's descriptor machinery continues as dynamic-β/drift-loop diagnostics unless later evidence beats Fixed-Share. v3.5 (2026-06-14): **M1c complete after M1b repo-hygiene snapshot (`m1b-finance-gate`)**. M1c ran a lightweight MISO breadth screen on ETTh1/ETTh2/ETTm1/ETTm2/Weather/Exchange (L=96,H=96) with a closed-form heterogeneous predictor pool plus anchors, Fixed-Share bounds, and descriptor→winner probes. Switch structure is broad (switch_ratio 0.076–0.280; oracle gaps 29.9–51.8%) but routability is mixed. v3.4 (2026-06-13): **M1b complete; gate adjudicated: ETT-only PRISM (§8 pivot clause)**. M1b ran the full amended protocol: DA/IC decision-loss oracle (killed), CryptoVol (real but bursty, fails strict condition 2), ETTh1 seed bands, expanded CryptoMISO pool, and FI2010 LOB (signal-bearing but fails condition 2). **Final M1b gate**: general-benchmark leg PASS; finance leg no surviving instantiation → **ETT-only PRISM** is the primary paper. CryptoVol and FI2010 results retained as appendix. v3.3 (2026-06-12) recorded the pre-FI2010 M1b adjudication. v3.2 (2026-06-12) recorded M1a results and amended gate protocol. v3.1 (2026-06-11) incorporated 2026 heterogeneous-MoE wave. v3.0 was the English rewrite. This document is the **single source of truth** for all subsequent coding and experiments.
>
...
- **Method status after M2**: the full learned-router claim is currently **not supported**. The empirical system is pivoted to frozen heterogeneous experts + **Fixed-Share** as the causal tracker, with PRISM descriptors retained for dynamic β, drift monitoring, interpretability, and future router repair.
