...
def main() -> None:
    ...
    gate_status = {
        ...
        "M2": {
            ...
            "fair_d0_rmse": m2.get("fair_controls_gamma_0_5", {}).get("d0_rmse_mean", "N/A"),
            "fair_d1_rmse": m2.get("fair_controls_gamma_0_5", {}).get("d1_rmse_mean", "N/A"),
            "fair_d2_rmse": m2.get("fair_controls_gamma_0_5", {}).get("d2_rmse_mean", "N/A"),
            "fair_d2_vs_d1_rmse_reduction": m2.get("fair_controls_gamma_0_5", {}).get("d2_vs_d1_rmse_reduction_mean", "N/A"),
        },
        "M3": {
            ...
            "m5_markdown_unit_p_report": m3.get("h5_m5_markdown", {}).get("unit_wilcoxon_p_report", "N/A"),
            "m5_markdown_nee_delta": m3.get("h5_m5_markdown", {}).get("paired_nee_delta", {}),
            ...
            "favorita_unit_p_report": m3.get("h5_favorita_promo", {}).get("unit_wilcoxon_p_report", "N/A"),
            "favorita_nee_delta": m3.get("h5_favorita_promo", {}).get("paired_nee_delta", {}),
    ...
    if gate_status["M2"]["h3_pass"]:
        ...
        headline_claims.append(
            ...
            f"hidden-confounder stress-test improvement in the current linear MISO ablation."
        )
    if not gate_status["M2"]["kill_ortho_inert"]:
        fair_d0 = gate_status["M2"]["fair_d0_rmse"]
        fair_d1 = gate_status["M2"]["fair_d1_rmse"]
        fair_d2 = gate_status["M2"]["fair_d2_rmse"]
        fair_red = gate_status["M2"]["fair_d2_vs_d1_rmse_reduction"]
        headline_claims.append(
            f"FAIR-CONTROL CHECK: when D0/D1/D2 all receive item controls, RMSE is "
            f"{fair_d0}/{fair_d1}/{fair_d2}; D2 reduces RMSE vs the fair D1 structural head "
            f"by {fair_red:.1%}. The controlled D0 result is reported separately and is not "
            "claimed as an orthogonal-loss win."
        )
    if gate_status["M3"]["h5_pass"]:
        ...
        p = gate_status["M3"]["favorita_unit_p_report"]
        robust_red = gate_status["M3"]["favorita_robust_median_reduction"]
        robust_p = gate_status["M3"]["favorita_robust_max_p_report"]
        fav_delta = gate_status["M3"]["favorita_nee_delta"]
        headline_claims.append(
            ...
            f"mean paired NEE delta {fav_delta.get('mean_delta', 'N/A')} "
            f"(95% CI {fav_delta.get('mean_delta_ci95', 'N/A')}); "
        ...
        mp = gate_status["M3"]["m5_markdown_unit_p_report"]
        md_delta = gate_status["M3"]["m5_markdown_nee_delta"]
        headline_claims.append(
            f"H5 REAL A-TYPE #2: M5 markdown NEE {md2} < D0 NEE {md0} "
            f"({mred:.1%} reduction; unit Wilcoxon p={mp}; "
            f"mean paired NEE delta {md_delta.get('mean_delta', 'N/A')}, "
            f"95% CI {md_delta.get('mean_delta_ci95', 'N/A')})."
        )
