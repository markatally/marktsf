...
version is a revised main-track candidate with an explicit TimeXer stability
caveat, not a guaranteed acceptance claim.
...
- **M4**: consolidated summary reports a revised main-track candidate after
  fair-control, split-hygiene, and reporting fixes.
- **M5**: strict local readiness audit returns `REVISED_MAIN_TRACK_CANDIDATE`.
- **M6**: fair-control D0/D1/D2 protocol strictly passes on DLinear, PatchTST,
  TiDE, and Transformer. TimeXer is a mean-pass boundary case because one seed
  exceeds the 5% WMAPE-degradation tolerance.
...
causal ground truth, HE-specific empirical validation, all-backbone strict
seed-level stability, TimeXer strict stability, or full leaderboard SOTA across
every TSF benchmark.
